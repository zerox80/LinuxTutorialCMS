...
async fn test_bug_fixes() -> anyhow::Result<()> {
    ...
    let summary: TutorialSummaryResponse = tutorials[0].try_into().expect("Should not fail even with invalid JSON");
    assert_eq!(summary.id, "test-1");
    assert!(summary.topics.is_empty(), "Should return empty topics for invalid JSON");
